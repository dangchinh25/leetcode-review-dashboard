...
async function main() {
  ...
  try {
    await prisma.$transaction(async (tx) => {
      ...
      const problemDetailsPromises = Object.entries(seedData).map(async ([questionId, data]) => {
      ...
      console.log('Fetching problem details... done', `(${problemDetails.length} problems)`, new Date().toISOString());

      console.log('Creating problems...', `(${problemDetails.length} problems)`, new Date().toISOString());
      ...
      console.log('Creating problems... done', `(${problems.length} problems)`, new Date().toISOString());
      ...
      console.log('Creating tags... ', `(${uniqueTags.size} tags)`, new Date().toISOString());
      ...
      console.log('Creating tags... done', `(${tags.length} tags)`, new Date().toISOString());

      // Create a map of tag slugs to tag records for quick lookup
      const tagMap = new Map(tags.map((tag) => [tag.slug, tag]));

      console.log('Creating problem-tag relationships and proficiency records...', `(${problemDetails.length} problems)`, new Date().toISOString());
